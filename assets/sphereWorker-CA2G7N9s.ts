(() => {
    ...
    let quantizeMesh: typeof import('./quantize').quantizeMesh;

    (async () => {
        const module = await import('./quantize');
        quantizeMesh = module.quantizeMesh;
    })();

    self.onmessage = async (e: MessageEvent<SphereData>) => {
        const { resolution, quantize } = e.data;

        // Wait for quantizeMesh to be loaded if needed
        if (quantize && !quantizeMesh) {
            while (!quantizeMesh) {
