...
async function generateInWorker<T>(workerUrl: URL, data: T): Promise<Mesh> {
...
export async function generateMesh(shape: ShapeType, resolution: number, quantize: boolean = false): Promise<Mesh> {
    const workerUrls = {
        sphere: new URL('./sphereWorker.ts', import.meta.url),
        wavySphere: new URL('./wavySphereWorker.ts', import.meta.url),
    };

    switch (shape) {
        case 'sphere':
            return generateInWorker(workerUrls.sphere, { resolution, quantize });
        case 'wavySphere':
            return generateInWorker(workerUrls.wavySphere, { resolution, quantize });
        case 'roundedBox':
...
export async function generateRoundedBox(resolution: number, quantize: boolean = false): Promise<Mesh> {
    const faces = [
        // Positive X
        {
